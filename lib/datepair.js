...
;(function( $, window, document, undefined ){
  ...
  DatepairPlugin.prototype = {
    ...
    init: function() {
      ...
      // Use a deep extend to merge changes to datepickerOpts and timepickerOpts
      ...
      this.dateInputs.each(function(){
        ...
        if ($this.hasClass('start') || $this.hasClass('end')) {
          $this.on('changeDate change', pluginInstance, pluginInstance.doDatepair);
        }
      ...
      this.timeInputs.each(function() {
        ...
        if ($this.hasClass('start') || $this.hasClass('end')) {
          $this.on('changeTime change', pluginInstance, pluginInstance.doDatepair);
        }
      ...
      if (startDateInput.length && endDateInput.length) {
        ...
        this.$elem.data('dateDelta', dateDelta);
      }
      ...
      if (startTimeInput.length && endTimeInput.length) {
        ...
        this.$elem.data('timeDelta', endInt - startInt);
    ...
    doDatepair: function(e) {
      ...
      if (target.hasClass('date') || target.attr('type') == 'date') {
        e.data.updateDatePair(target);

      } else if (target.hasClass('time') || target.attr('type') == 'time') {
        e.data.updateTimePair(target);
      }
    },

    updateDatePair: function(target) {
      var start = this.$elem.find('input.start.date');
      var end = this.$elem.find('input.end.date');
      if (!start.length || !end.length) {
        return;
      }

      var startDate = this.parseDate(start.val(), this.config.datepickerOpts['format']);
      var endDate =  this.parseDate(end.val(), this.config.datepickerOpts['format']);

      var oldDelta = this.$elem.data('dateDelta');
      ...
      if (!isNaN(oldDelta) && oldDelta !== null && target.hasClass('start')) {
        ...
        if (newDelta < 86400000) {
          var startTimeVal = this.$elem.find('input.start.time').val();

          if (startTimeVal) {
            this.$elem.find('input.end.time').timepicker('option', {'minTime': startTimeVal});
          }
        } else {
          this.$elem.find('input.end.time').timepicker('option', {'minTime': null});
        }

        this.$elem.data('dateDelta', newDelta);
      }
    },

    updateTimePair: function(target) {
      var start = this.$elem.find('input.start.time');
      var end = this.$elem.find('input.end.time');
      ...
      var dateDelta = this.$elem.data('dateDelta');
      ...
      var oldDelta = this.$elem.data('timeDelta');
      ...
      this.$elem.data('timeDelta', newDelta);
      ...
      var startInput = this.$elem.find('.start.date');
      var endInput = this.$elem.find('.end.date');

      if (startInput.val() && !endInput.val()) {
        endInput.val(startInput.val());
        endInput.datepicker('update');
        dateDelta = 0;
        this.$elem.data('dateDelta', 0);
      }
      ...
      if (endDateAdvance != 0) {
        if (dateDelta || dateDelta === 0) {
          ...
          endInput.datepicker('update');
          this.$elem.data('dateDelta', dateDelta + endDateAdvance);
        }
      }
    },
