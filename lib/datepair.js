$(function() {
	var DATEPICKER_FORMAT = 'yyyy-m-d'; // for this format see the parseDate function below. Must be analogous to the DATE_FORMAT.
	var TIMEPICKER_FORMAT = 'g:ia';
	var DATE_FORMAT = 'Y-n-j'; // for this format see http://php.net/manual/function.date.php . Must be analogous to the DATEPICKER_FORMAT.
	...
	function updateDatePair(target, container)
	{
		...
		if (!isNaN(oldDelta) && oldDelta !== null && target.hasClass('start')) {
			...
			var startTime = container.find('input.start.time');
			var endTime = container.find('input.end.time');
			var lessThan1Day = (newDelta < 86400000);
			if (lessThan1Day && startTime.length && endTime.length && startTime.val()) {
				endTime.timepicker('option', {'minTime': startTime.val()});
			} else if (!lessThan1Day && endTime.length) {
				endTime.timepicker('option', {'minTime': null});
			}
